...
public:
	...
	MinesweeperWindow(int x, int y, int width, int height, int mines_, const string& title);
	int randomCell(int upperLim , int lowerLim);
	int tilesOpen = 0;
private:
	const int width;		// Bredde i antall tiles
	const int height;		// Hoyde i antall tiles
	const int mines;		// Antall miner
	TDT4102::TextInput status{{300,0} , 300 , 50 , "Status Window"};
